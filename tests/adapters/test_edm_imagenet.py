"""Tests for EDM ImageNet adapter.
...
@pytest.fixture
def mock_adapter():
    """Create mock EDM adapter for testing API."""
    from adapt_diff.adapters.edm_imagenet import EDMImageNetAdapter
    ...
    model = MockEDMModel()
    return EDMImageNetAdapter(model, device='cpu')


class TestEDMImageNetAdapterProperties:
...
class TestEDMImageNetAdapterDiffusionMethods:
    ...
    def test_get_initial_noise(self, mock_adapter):
        ...
        # Check it's scaled by sigma_max (default 80.0)
    ...
    def test_forward_with_cfg(self, mock_adapter):
        ...
        t = torch.tensor([10.0])
        cond = torch.eye(1000)[281:282]  # class 281
